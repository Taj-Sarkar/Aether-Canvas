...
export const AppShell = ({ user, onLogout }: AppShellProps) => {
  ...
  const [activeTab, setActiveTab] = useState<
    "chat" | "breakdown" | "viz" | "agents" | "flashcards"
  >("chat");
    ...
    chatHistory: [
      {
        id: "0",
        role: "model",
        content: "Hello! I'm ready to help you organize this workspace.",
        timestamp: Date.now(),
      },
    ],
    breakdown: null,
  ...
  const [agents, setAgents] = useState<Record<string, AgentState>>({
    ...
    [AgentType.STRUCTURER]: {
      type: AgentType.STRUCTURER,
      status: AgentStatus.IDLE,
    },
    [AgentType.DATA_VIZ]: {
      type: AgentType.DATA_VIZ,
      status: AgentStatus.IDLE,
    },
  ...
  const updateAgent = (
    ...
    setAgents((prev) => ({
      ...prev,
      [type]: { ...prev[type], status, message },
    }));
  };

  const resetAgents = () => {
    Object.values(AgentType).forEach((t) =>
      updateAgent(t as AgentType, AgentStatus.IDLE),
    );
  };
  ...
  const setWorkspaceSlice = (
    updater: (data: WorkspaceData) => WorkspaceData,
  ) => {
    setWorkspaceData((prev) => ({
      ...prev,
      [activeWorkspaceId]: updater(
        prev[activeWorkspaceId] || initialWorkspaceData,
      ),
    }));
  };
  ...
  const handleAddBlock = (type: BlockType) => {
    ...
    if (type === BlockType.DATASET) {
      ...
      (newBlock as DatasetBlock).description =
        "Monthly sales data for North America region";
    }
  ...
  const handleSaveLastResponseToNote = () => {
    const lastModel = [...currentData.chatHistory]
      .reverse()
      .find((m) => m.role === "model");
    if (!lastModel) return;
    ...
    try {
      ...
      if (isDataUrl) {
        ...
        if (block.src.includes("picsum")) {
          ...
          description =
            "The image appears to be a random landscape or abstract scene (placeholder). It contains natural colors and lighting.";
  ...
  const handleRecommendCharts = async (block: DatasetBlock) => {
    ...
    try {
      const config = await generateChartRecommendation(
        block.description || "Generic dataset",
      );
      setWorkspaceSlice((data) => ({
        ...data,
        visualizations: [config, ...data.visualizations],
      }));
  ...
  const sendChat = async (message: string) => {
    ...
    const context = currentData.blocks
      .map((b) => {
        if (b.type === BlockType.TEXT)
          return `Note (${b.title}): ${(b as TextBlock).content}`;
        if (b.type === BlockType.DATASET)
          return `Dataset (${b.title}): ${(b as DatasetBlock).description}`;
    ...
    try {
      const responseText = await chatWithWorkspace(
      ...
      setWorkspaceSlice((data) => ({
        ...data,
        chatHistory: [...data.chatHistory, modelMsg],
      }));
    } catch (e: any) {
      ...
      setWorkspaceSlice((data) => ({
        ...data,
        chatHistory: [...data.chatHistory, errorMsg],
      }));
    } finally {
  ...
  const handleGenerateFlashcards = async () => {
    ...
    try {
      ...
      try {
        ...
        const flashcardBlocks = responseText
          .split("---")
  ...
  return (
    <div className="app-shell-root">
      ...
      <div className="app-container">
        {/* LEFT SIDEBAR */}
        <aside
          className={`sidebar-left glass-panel${isSidebarOpen ? " mobile-open" : ""}`}
        >
          <div>
            ...
            <div className="nav-group">
              ...
              <div
                className="nav-item new-workspace"
                ...
                >
                  <div
                    style={{
                      ...
                      borderRadius: "50%",
                      background:
                        "linear-gradient(135deg, var(--app-accent-primary), var(--app-accent-secondary))",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      color: "white",
                  ...
                  <div>
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          {(block as DatasetBlock).fileName}
                        </p>
                        <p
                          style={{
                            fontSize: "0.85rem",
                        ...
                        >
                          {(block as DatasetBlock).rowCount} rows •{" "}
                          {(block as DatasetBlock).columns.join(", ")}
                        </p>
                        <p
                          style={{
                            fontSize: "0.75rem",
                        ...
                        >
                          {(block as DatasetBlock).description}
                        </p>
                      </div>
                    </div>
                    <div className="ai-actions">
                      <div
                        className="ai-chip"
                        onClick={() =>
                          handleRecommendCharts(block as DatasetBlock)
                        }
                      >
        ...
        {/* RIGHT SIDEBAR (AI) */}
        <aside className={`sidebar-right glass-panel${isAIPanelOpen ? ' mobile-open' : ''}`}>
                ...
                {chatHistory.map((msg) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        className="msg-bubble markdown-content"
                        style={{ flex: 1 }}
                        dangerouslySetInnerHTML={{
            ...
            <div className="tab-content">
              {!breakdown ? (
                <div className="empty-state">
                  ...
                  <p>
                    Select a text block and click "Analyze" to see a structured
                    breakdown.
                  </p>
                </div>
              ) : (
                ...
                >
                  <div className="note-card">
                    ...
                    <p
                      style={{ color: "var(--app-text-main)", lineHeight: 1.6 }}
                    >
                      {breakdown.summary}
                    </p>
                  </div>
                  ...
                  >
                    {(breakdown.tags || []).map((tag) => (
                      <span key={tag} className="ai-chip">
                        #{tag}
                      </span>
                    ))}
                  </div>
            ...
            <div className="tab-content">
              {visualizations.length === 0 ? (
                <div className="empty-state">
                  ...
                  <p>
                    Upload a dataset and click "Recommend Charts" to generate
                    visuals.
                  </p>
                </div>
              ) : (
              ...
              {flashcards.length === 0 ? (
                <div className="empty-state">
                  ...
                  <p style={{ fontSize: "0.75rem" }}>
                    Click "Generate" to create flashcards from your workspace
                    content.
                  </p>
                </div>
              ) : (
                      ...
                      >
                        <div
                          style={{
                            ...
                            borderLeft: "4px solid var(--app-accent-primary)",
                            padding: "12px",
                            borderRadius: "6px",
                          }}
                        >
      ...
      >
        {isAIPanelOpen ? <Icons.X size={22} /> : <Icons.Sparkles size={22} />}
      </button>
